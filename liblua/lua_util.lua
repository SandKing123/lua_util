...
function lua_util.intervalDays(time1, time2)
	return math.floor(((time2 or utime.now()) - _BeginDayTime()) / 86400) - math.floor((time1 - _BeginDayTime()) / 86400)
end
os.intervalDays = lua_util.intervalDays

function lua_util.dayEndTime(time)
	return math.ceil(((time or utime.now()) - _BeginDayTime()) / 86400) * 86400 + _BeginDayTime()
end
...
function lua_util.intervalWeeks(time1, time2)
	return math.floor(((time2 or utime.now()) - _BeginWeekTime()) / 86400) - math.floor((time1 - _BeginWeekTime()) / 86400)
end
os.intervalWeeks = lua_util.intervalWeeks

function lua_util.weekEndTime(time)
	return math.ceil(((time or utime.now()) - _BeginWeekTime()) / 604800) * 604800 + _BeginWeekTime()
end
os.weekEndTime = lua_util.weekEndTime

function lua_util.getTimestamp(strtime)
	local time = lua_util.split(strtime,":")
	local t = os.date("*t", utime.now())
